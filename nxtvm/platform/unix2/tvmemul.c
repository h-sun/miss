...
#include "tvmemul.h"//to do

#define MEMORY_SIZE 8192 /* 16 Kb */
#define EXTRA_MEMORY_SIZE 1536 /* 3 Kb */
#define DEBUG_RUNS  0//to do:debugging@2011.07.05
//#define DEBUG_RCX_MEMORY    1
				      ...
				      const int pc)
{
    ...
    printf ("--  Exception class   : %u\n", (unsigned) get_class_index (&(exception->_super)));
    printf ("--  Thread            : %u\n", (unsigned) thread->threadId);
    printf ("--  Method No.  : %u\n", (unsigned) methodRecord);//to do:rewrite accordding to main.c in the nxt version
...
void run(void)
{
  ...
  //perror ("Created bootThread: %d. Initializing...\n");
  #endif
